...
namespace math {
	...
	__forceinline qangle_t vector_angles(const vec3_t& in)
	{
		qangle_t out{};
	...
	__forceinline void vector_transform( vec3_t& in, mat3x4_t& matrix, vec3_t& out ) {
		out = {
	...
	__forceinline vec3_t vector_transform(
		const vec3_t& in,
		const mat3x4_t& matrix
	) {
		return {
			in.dot({ matrix[0][0], matrix[0][1], matrix[0][2] }) + matrix[0][3],
			in.dot({ matrix[1][0], matrix[1][1], matrix[1][2] }) + matrix[1][3],
	...
	__forceinline void vector_rotate( vec3_t& in, mat3x4_t& matrix, vec3_t& out ) {
		out = { in.dot( matrix[ 0 ] ), in.dot( matrix[ 1 ] ), in.dot( matrix[ 2 ] ) };
	}
	__forceinline vec3_t vector_rotate ( vec3_t& in1, mat3x4_t& in2 )
	{
		return vec3_t ( in1.dot ( in2[ 0 ] ), in1.dot ( in2[ 1 ] ), in1.dot ( in2[ 2 ] ) );
	}
	__forceinline vec3_t vector_rotate ( vec3_t& in1, qangle_t& in2 )
	{
	    auto matrix = angle_matrix ( in2 );
	...
	__forceinline void vector_matrix(vec3_t& forward, mat3x4_t& out) {
		vec3_t right, up;

		if (forward.x == 0.f && forward.y == 0.f) 
		{
			right = { 0.f, -1.f, 0.f };
			up = { -forward.z, 0.f, 0.f };
		}
		else 
		{
			right = forward.cross(vec3_t(0.f, 0.f, 1.f)).normalized();
	...
	__forceinline mat3x4_t vector_matrix( vec3_t& in ) {
		vec3_t right{}, up{};
