...
#include "../../cloud/worker/worker.h"

BOOL WINAPI DllMain(HMODULE m_module_address, DWORD reason_for_call, LPVOID reserved)
{
...
namespace nem {
	...
	void c_context::init ( )
	{
		...
		const auto d3d9_device = **reinterpret_cast< IDirect3DDevice9*** >(
			find_byte_seq( GetModuleHandle (  ( "shaderapidx9.dll" ) ), "A1 ? ? ? ? 50 8B 08 FF 51 0C" ) + 1u
			);
		...
		/* init imgui */
		{
			...
			render::m_draw_list = std::make_unique<ImDrawList>(ImGui::GetDrawListSharedData());
			render::m_data_draw_list = std::make_unique<ImDrawList>(ImGui::GetDrawListSharedData());
			render::m_replace_draw_list = std::make_unique<ImDrawList>(ImGui::GetDrawListSharedData());
			...
			hacks::g_visuals->m_fonts.m_tahoma14 = io.Fonts->AddFontFromFileTTF( ( "C:\\Windows\\Fonts\\Tahoma.ttf" ), 14.f, &cfg );

			hacks::g_visuals->m_fonts.m_verdana = io.Fonts->AddFontFromFileTTF( ( "C:\\Windows\\Fonts\\Verdana.ttf" ), 12.f, &cfg );
			...
			hacks::g_visuals->m_fonts.m_log_font = io.Fonts->AddFontFromFileTTF( ( "C:/windows/fonts/seguisb.ttf" ), 16.0f, &cfg );
			hacks::g_visuals->m_fonts.m_mojesh_ubrat_ee_ya_razreshayu = io.Fonts->AddFontFromFileTTF( ( "C:/windows/fonts/seguisb.ttf" ), 30.0f, &cfg );
			...
			hacks::g_visuals->m_fonts.m_segoe_ui_esp = io.Fonts->AddFontFromFileTTF( ( "C:/windows/fonts/seguisb.ttf" ), 13.0f, &cfg );
		...
		const auto cl_code_section = code_section_t { GetModuleHandle (  ( "client.dll" ) ) };
		const auto eng_code_section = code_section_t { GetModuleHandle (  ( "engine.dll" ) ) };
		const auto vgui_code_section = code_section_t { GetModuleHandle (  ( "vguimatsurface.dll" ) ) };
		const auto vgui2_code_section = code_section_t { GetModuleHandle (  ( "vgui2.dll" ) ) };
		const auto server_code_section = code_section_t { GetModuleHandle (  ( "server.dll" ) ) };
		const auto studio_render_code_section = code_section_t { GetModuleHandle (  ( "studiorender.dll" ) ) };
		const auto material_system_code_section = code_section_t { GetModuleHandle (  ( "materialsystem.dll" ) ) };
		const auto shader_pidx9_code_section = code_section_t { GetModuleHandle (  ( "shaderapidx9.dll" ) ) };
		const auto vstdlib_code_section = code_section_t { GetModuleHandle (  ( "vstdlib.dll" ) ) };
		...
		/* init valve stuff */
		{
			...
			valve::g_localize = reinterpret_cast< valve::c_localize * ( __cdecl* )( const char*, int* ) >(
				GetProcAddress ( GetModuleHandle (  ( "localize.dll" ) ),  ( "CreateInterface" ) )
				)(  ( "Localize_001" ), nullptr );
			...
			valve::g_model_render = reinterpret_cast< valve::c_model_render * ( __cdecl* )( const char*, int* ) >(
				GetProcAddress ( GetModuleHandle (  ( "engine.dll" ) ),  ( "CreateInterface" ) )
				)(  ( "VEngineModel016" ), nullptr );

			valve::g_mdl_cache = reinterpret_cast< valve::c_mdl_cache * ( __cdecl* )( const char*, int* ) >(
				GetProcAddress ( GetModuleHandle (  ( "datacache.dll" ) ),  ( "CreateInterface" ) )
				)(  ( "MDLCache004" ), nullptr );
			...
			g_worker->username = SteamFriends->GetPersonaName( );
		...
		/* init addresses */
		{
			...
			m_addresses.m_random_float = reinterpret_cast< addresses_t::random_float_t >( GetProcAddress( vstdlib_dll, ( "RandomFloat" ) ) );
			m_addresses.m_random_int = reinterpret_cast< addresses_t::random_int_t >( GetProcAddress ( vstdlib_dll,  ( "RandomInt" ) ) );
			m_addresses.m_random_seed = reinterpret_cast< addresses_t::random_seed_t >( GetProcAddress ( vstdlib_dll,  ( "RandomSeed" ) ) );
			...
			//m_addresses.m_ayo_who_asked =  ( addresses_t::who_asked_fn )find_byte_seq(cl_code_section, ("55 8B EC 83 EC 0C 53 56 8B 71 58"));
		}
		...
		/* init cvars */
		{
			m_cvars.m_r_jiggle_bones = valve::g_cvar->find_var (  ( "r_jiggle_bones" ) );
			m_cvars.m_cl_interp = valve::g_cvar->find_var (  ( "cl_interp" ) );
			m_cvars.m_cl_interp_ratio = valve::g_cvar->find_var (  ( "cl_interp_ratio" ) );
			m_cvars.m_cl_updaterate = valve::g_cvar->find_var (  ( "cl_updaterate" ) );
			m_cvars.m_ff_dmg_reduction_bullets = valve::g_cvar->find_var (  ( "ff_damage_reduction_bullets" ) );
			m_cvars.m_ff_dmg_bullet_pen = valve::g_cvar->find_var (  ( "ff_damage_bullet_penetration" ) );
			m_cvars.m_sv_auto_bhop = valve::g_cvar->find_var (  ( "sv_autobunnyhopping" ) );
			m_cvars.m_sv_clockcorrection_msecs = valve::g_cvar->find_var (  ( "sv_clockcorrection_msecs" ) );
			m_cvars.m_sv_maxvelocity = valve::g_cvar->find_var (  ( "sv_maxvelocity" ) );
			m_cvars.m_sv_friction = valve::g_cvar->find_var (  ( "sv_friction" ) );
			m_cvars.m_sv_accelerate = valve::g_cvar->find_var (  ( "sv_accelerate" ) );
			m_cvars.m_sv_accelerate_use_weapon_speed = valve::g_cvar->find_var (  ( "sv_accelerate_use_weapon_speed" ) );
			m_cvars.m_weapon_accuracy_nospread = valve::g_cvar->find_var (  ( "weapon_accuracy_nospread" ) );
			m_cvars.m_sv_maxunlag = valve::g_cvar->find_var (  ( "sv_maxunlag" ) );
			m_cvars.m_sv_enable_bhop = valve::g_cvar->find_var (  ( "sv_enablebunnyhopping" ) );
			m_cvars.m_sv_jump_impulse = valve::g_cvar->find_var (  ( "sv_jump_impulse" ) );
			m_cvars.m_sv_gravity = valve::g_cvar->find_var (  ( "sv_gravity" ) );
			m_cvars.m_weapon_recoil_scale = valve::g_cvar->find_var (  ( "weapon_recoil_scale" ) );
			m_cvars.m_weapon_accuracy_shotgun_spread_patterns = valve::g_cvar->find_var (  ( "weapon_accuracy_shotgun_spread_patterns" ) );
			m_cvars.m_molotov_throw_detonate_time = valve::g_cvar->find_var (  ( "molotov_throw_detonate_time" ) );
			m_cvars.m_weapon_molotov_maxdetonateslope = valve::g_cvar->find_var (  ( "weapon_molotov_maxdetonateslope" ) );
			m_cvars.m_cl_fullupdate = valve::g_cvar->find_var (  ( "cl_fullupdate" ) );
			m_cvars.m_sv_stopspeed = valve::g_cvar->find_var (  ( "sv_stopspeed" ) );
		}
...
#endif
		{
			auto& vel_mod_prop = std::get< valve::recv_prop_t* > ( valve::g_net_vars->entry (  ( "CCSPlayer->m_flVelocityModifier" ) ) );
			...
			auto& sequence_prop = std::get< valve::recv_prop_t* > ( valve::g_net_vars->entry (  ( "CBaseViewModel->m_nSequence" ) ) );
		...
		if ( MH_CreateHook (
			( *reinterpret_cast< LPVOID** >( valve::g_cvar->find_var (  ( "sv_cheats" ) ) ) )[ 13u ],
		...
		if ( MH_CreateHook (
			reinterpret_cast< LPVOID >(
				find_byte_seq (
					GetModuleHandle (  ( "materialsystem.dll" ) ),
		...
		auto fp = fopen( ( "threads_log.txt" ), "wb" );
		fprintf( fp, ( "threads created: %d" ), std::jthread::hardware_concurrency( ) );
		fclose( fp );
	}
