...
namespace nem::hacks {
	...
	) const
	{
		...
		static auto mp_dmg_scale_ct_head = valve___g_cvar->find_var (  ( "mp_damage_scale_ct_head" ) );
		static auto mp_dmg_scale_t_head = valve___g_cvar->find_var (  ( "mp_damage_scale_t_head" ) );

		static auto mp_dmg_scale_ct_body = valve___g_cvar->find_var (  ( "mp_damage_scale_ct_body" ) );
		static auto mp_dmg_scale_t_body = valve___g_cvar->find_var (  ( "mp_damage_scale_t_body" ) );
	...
	) const
	{
		...
		while ( cur_dmg > 0.f ) {
			...
			if ( trace.m_fraction == 1.f
				|| ( trace.m_end_pos - src ).length ( ) > max_dist.length( ) )
				break;
