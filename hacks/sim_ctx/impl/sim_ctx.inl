...
namespace nem::hacks {
	__forceinline void c_sim_context::check_velocity ( extrapolation_data_t& data ) const {
		static auto sv_max_vel = valve___g_cvar->find_var (  ( "sv_maxvelocity" ) );
	...
	__forceinline void c_sim_context::friction ( extrapolation_data_t& data ) const {
		static auto sv_friction = valve___g_cvar->find_var (  ( "sv_friction" ) );
		static auto sv_stopspeed = valve___g_cvar->find_var (  ( "sv_stopspeed" ) );
	...
	__forceinline void c_sim_context::air_accelerate ( extrapolation_data_t& data, vec3_t& wish_dir, float wish_spd ) const {
		static auto sv_airaccelerate = valve___g_cvar->find_var (  ( "sv_airaccelerate" ) );
	...
	__forceinline void c_sim_context::walk_move ( extrapolation_data_t& data ) const {
		...
		static auto sv_stepsize = valve___g_cvar->find_var (  ( "sv_stepsize" ) );
		valve::trace_t trace;
		static auto sv_accelerate = valve___g_cvar->find_var (  ( "sv_accelerate" ) );
		...
		accelerate ( data, wish_dir, wish_spd.length( ), sv_accelerate->get_float( ) );
		data.m_velocity.z = 0.f;

		static auto sv_maxspeed = valve___g_cvar->find_var (  ( "sv_maxspeed" ) );
