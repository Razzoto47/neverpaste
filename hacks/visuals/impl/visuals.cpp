...
namespace nem::hacks {
	...
	bool c_visuals::add_grenade_simulation( const grenade_simulation_t& sim, const bool warning ) const {
		...
		if ( warning ) {
			...
			render::rect_filled ( screen_pos - vec2_t ( 12, 5 ), vec2_t ( 42.f, 42.f ), col_t ( 30.f, 30.f, 30.f, 255.f ), 20.f, true );
			render::rect ( screen_pos - vec2_t ( 12, 5 ), vec2_t ( 42.f, 42.f ), col_t ( 235, 235, 235, 255 ), 20.f, true );
	...
	void c_visuals::override_material(
		...
		static const auto glow_overlay = valve___g_mat_system->find_material (  ( "dev/glow_armsrace" ), nullptr );
	...
	void c_visuals::add_beam_ring( const vec3_t& pos, const float* const clr ) const {
		...
		info.m_model_index = valve___g_model_info->lookup_model_index( ( "sprites/purplelaser1.vmt" ) );
	...
	void molly ( valve::c_entity* ent )
	{
		...
		if ( render::world_to_screen ( origin, screen_origin ) )
		{
			...
			render::text (  ( "MOLLY" ), vec2_t ( screen_origin.x, screen_origin.y - size.y * 0.5f + 12.0f ), col_t ( 255, 255, 255, 255 ), hacks::g_visuals->m_fonts.m_skeet_font_esp, FONT_OUTLINE | FONT_CENTERED_X );
		}
	}
	...
	void c_visuals::handle_projectile( valve::c_entity* const entity, const valve::e_class_id class_id ) {
		...
		case valve::e_class_id::molotov_projectile:
			render::text (  ( "MOLOTOV" ), vec2_t ( screen_pos.x, screen_pos.y ), col_t ( 255, 255, 255, 255 ), m_fonts.m_skeet_font_esp, FONT_OUTLINE );
			break;

		case valve::e_class_id::decoy_projectile:
			render::text (  ( "DECOY" ), vec2_t ( screen_pos.x, screen_pos.y ), col_t ( 255, 255, 255, 255 ), m_fonts.m_skeet_font_esp, FONT_OUTLINE );
			break;
		...
		case valve::e_class_id::base_cs_grenade_projectile:
			if ( const auto model = entity->model( ) ) {
				...
				if ( name.find( ( "flashbang" ) ) != std::string::npos )
					render::text (  ( "FLASH" ), vec2_t ( screen_pos.x, screen_pos.y ), col_t ( 255, 255, 255, 255 ), m_fonts.m_skeet_font_esp, FONT_OUTLINE );

				else if ( name.find( ( "fraggrenade" ) ) != std::string::npos )
					render::text (  ( "FRAG" ), vec2_t ( screen_pos.x, screen_pos.y ), col_t ( 255, 255, 255, 255 ), m_fonts.m_skeet_font_esp, FONT_OUTLINE );
			}
		...
		case valve::e_class_id::smoke_grenade_projectile:
			render::text (  ( "SMOKE" ), vec2_t ( screen_pos.x, screen_pos.y ), col_t ( 255, 255, 255, 255 ), m_fonts.m_skeet_font_esp, FONT_OUTLINE );
	...
	{
		static auto sv_skyname = valve___g_cvar->find_var(("sv_skyname"));
	...
	void c_visuals::draw_c4_status ( bool im_gui_suck )
	{
		...
		if ( !hacks::g_aim_bot->cfg ( ).m_bBombIndicator )
			return;
		...
		ImGui::Begin (  ( "C4 hueta " ), &hacks::g_aim_bot->cfg ( ).m_bC4Indicator, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | advanced_flag );
		{
			int32_t x = ImGui::GetCurrentWindow ( )->Pos.x;
			int32_t y = ImGui::GetCurrentWindow ( )->Pos.y;

			ImGui::PushFont ( m_fonts.m_warning_icon_font );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 8, y + 10 ), ImColor ( 255, 255, 255 ),  ( "o" ) );
			ImGui::PopFont ( );
			...
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 40, y + 16 ), ImColor ( 255, 255, 255 ),  ( "Damage: " ) );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 94, y + 16 ), ImColor ( 255, 255, 255 ), str.c_str ( ) );

			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 123, y + 16 ), ImColor ( 255, 255, 255 ),  ( "Time: " ) );
	...
	void c_visuals::draw_spectators ( bool im_gui_suck )
	{
		...
		ImGui::SetWindowPos( ImVec2 ( 200, 250 ) );
		ImGui::Begin (  ( "Spectators " ), &hacks::g_aim_bot->cfg ( ).m_bSpectators, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | advanced_flag );
		{
			int32_t x = ImGui::GetCurrentWindow ( )->Pos.x;
			int32_t y = ImGui::GetCurrentWindow ( )->Pos.y;

			ImGui::GetForegroundDrawList ( )->AddRectFilled ( ImVec2 ( x, y ), ImVec2 ( x + 181, y + 20 ), col_t ( 35, 35, 35, 255 ).hex ( ), 3.f );

			ImGui::PushFont ( m_fonts.m_xxxtentacion_icons );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 6, y + 2 ), ImColor ( 255, 255, 255 ),  ( "b" ) );
			ImGui::PopFont ( );

			ImGui::PushFont ( m_fonts.m_museo_sans_name );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 20, y + 1 ), ImColor ( 255, 255, 255 ),  ( "| " ) );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 27, y + 2 ), ImColor ( 255, 255, 255 ),  ( "Spectators " ) );
			ImGui::PopFont ( );
			...
			{
				ImGui::PushFont ( m_fonts.m_museo_sans_name );
				ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 5, 23 + ( y + 16 * m_Last ) ), ImColor ( 255, 255, 255, 255 ), spectator.c_str ( ) );
				ImGui::PopFont ( );
	...
	void c_visuals::draw_key_binds ( bool imgui_suck )
	{
		...
		ImGui::Begin (  ( "Keybind List" ), &hacks::g_aim_bot->cfg ( ).m_bKeybinds, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | advanced_flag );
		{
			int32_t x = ImGui::GetCurrentWindow ( )->Pos.x;
			int32_t y = ImGui::GetCurrentWindow ( )->Pos.y;

			ImGui::GetForegroundDrawList( )->AddRectFilled ( ImVec2 ( x, y ), ImVec2 ( x + 181, y + 20 ), col_t ( 35, 35, 35, 255 ).hex ( ), 3 );

			ImGui::PushFont ( m_fonts.m_xxxtentacion_icons );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 6, y + 2 ), ImColor ( 255, 255, 255 ),  ( "a" ) );
			ImGui::PopFont ( );

			ImGui::PushFont ( m_fonts.m_museo_sans_name );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 20, y + 1 ), ImColor ( 255, 255, 255 ),  ( "| " ) );
			ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 27, y + 2 ), ImColor ( 255, 255, 255 ),  ( "Keybinds " ) );
			ImGui::PopFont ( );
			...
			for ( auto& bind : m_binds )
			{
				...
				ImGui::PushFont ( m_fonts.m_museo_sans_name );
				ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( x + 5, 27 + ( y + 16 * m_last ) ), ImColor ( 255, 255, 255, static_cast < int >( bind.second.m_alpha_modifier * 255.0f ) ), bind.second.m_name.c_str ( ) );
				ImGui::GetForegroundDrawList( )->AddText( ImVec2( x + 157, 27 + ( y + 16 * m_last ) ), ImColor( 255, 255, 255, static_cast < int >( bind.second.m_alpha_modifier * 255.0f ) ), bind.second.m_state.c_str( ) );
				ImGui::PopFont ( );
	...
	{
		if ( !hacks::g_aim_bot->cfg ( ).m_bWatermark )
			return;

		std::string name = SteamFriends->GetPersonaName( );
		std::string water_mark =  ( "neverlose | " ) + name +  ( " |" );
		...
		if ( valve___g_engine->in_game ( ) )
		{
			if ( net_channel )
			{
				...
				if ( latency )
				{
					static auto cl_updaterate = valve___g_cvar->find_var (  ( "cl_updaterate" ) );
		...
		water_mark +=  ( " fps: " ) + std::to_string ( last_fps );
		...
		ImGui::PushFont ( m_fonts.m_museo_sans_name );
		ImGui::GetForegroundDrawList ( )->AddText ( ImVec2 ( screen_size_x - text_length - 23 + 0, 14 ), ImColor ( 255, 255, 255, 255 ), water_mark.c_str ( ) );
		ImGui::PopFont ( );
	}
...
{ \
	...
	void c_visuals::on_render_start( ) {
		...
		**reinterpret_cast< bool** >(
		...
		static auto cl_wpn_sway_amt = valve___g_cvar->find_var (  ( "cl_wpn_sway_scale" ) );
		...
		static auto cl_foot_contact_shadows = valve___g_cvar->find_var (  ( "cl_foot_contact_shadows" ) );
		static auto cl_foot_contact_shadows_ = valve___g_cvar->find_var (  ( "cl_csm_static_prop_shadows" ) );
		static auto cl_foot_contact_shadows_1 = valve___g_cvar->find_var (  ( "cl_csm_world_shadows" ) );
		static auto cl_foot_contact_shadows_2 = valve___g_cvar->find_var (  ( "cl_csm_shadows" ) );
		static auto cl_foot_contact_shadows_3 = valve___g_cvar->find_var (  ( "cl_csm_viewmodel_shadows" ) );
		static auto cl_foot_contact_shadows_4 = valve___g_cvar->find_var (  ( "cl_csm_sprite_shadows" ) );
		static auto cl_foot_contact_shadows_5 = valve___g_cvar->find_var (  ( "cl_csm_rope_shadows" ) );
		...
		static const std::array< valve::material_t*, 4u > smoke_materials{
			valve___g_mat_system->find_material( ( "particle/vistasmokev1/vistasmokev1_fire" ), nullptr ),
			valve___g_mat_system->find_material( ( "particle/vistasmokev1/vistasmokev1_smokegrenade" ), nullptr ),
			valve___g_mat_system->find_material( ( "particle/vistasmokev1/vistasmokev1_emods" ), nullptr ),
			valve___g_mat_system->find_material( ( "particle/vistasmokev1/vistasmokev1_emods_impactdust" ), nullptr )
		};

		static auto enable_fog = valve___g_cvar->find_var (  ( "fog_enable" ) );
		static auto override_fog = valve___g_cvar->find_var (  ( "fog_override" ) );
		static auto fog_clr = valve___g_cvar->find_var (  ( "fog_color" ) );
		static auto fog_start = valve___g_cvar->find_var (  ( "fog_start" ) );
		static auto fog_end = valve___g_cvar->find_var (  ( "fog_end" ) );
		static auto fog_density = valve___g_cvar->find_var (  ( "fog_maxdensity" ) );
		static auto cl_ragdoll_disable = valve___g_cvar->find_var (  ( "cl_ragdoll_physics_enable" ) );
		...
		static const std::array< valve::material_t*, 2u > flash_materials{
			valve___g_mat_system->find_material( ( "effects/flashbang" ), nullptr ),
			valve___g_mat_system->find_material( ( "effects/flashbang_white" ), nullptr )
		};
		...
		static auto blur_overlay = valve___g_mat_system->find_material (  ( "dev/scope_bluroverlay" ),  ( "Other textures" ) );
		static auto scope_dirt = valve___g_mat_system->find_material (  ( "models/weapons/shared/scope/scope_lens_dirt" ),  ( "Other textures" ) );
	...
	void c_player_esp::draw_box ( valve::c_player* player, RECT& rect ) {
		...
		auto color = col_t ( 
			hacks::g_aim_bot->cfg ( ).m_flBoxColor[ 0 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flBoxColor[ 1 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flBoxColor[ 2 ] * 255.f,
			m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flBoxColor[ 3 ] );
	...
	void c_player_esp::draw_avatar ( valve::c_player* player, RECT& rect ) {
		if ( hacks::g_aim_bot->cfg ( ).m_iAvatarType < 0 )
			return;

		auto color = col_t ( 255, 255, 255, ( int ) m_dormant_data [ player->index ( ) ].m_alpha );

		switch ( hacks::g_aim_bot->cfg ( ).m_iAvatarType )
		{
		case 1:
			render::draw_image( g_NL->m_pIcons[IC_FOREX_ESP], rect.left, rect.top, rect.right, rect.bottom, vec2_t( 0, 0 ), vec2_t( 1, 1 ), color );
			break;
		case 2:
			render::draw_image( g_NL->m_pIcons[IC_LEBUBU_ESP], rect.left, rect.top, rect.right, rect.bottom, vec2_t( 0, 0 ), vec2_t( 1, 1 ), color );
			break;
		case 3:
			render::draw_image( g_NL->m_pIcons[IC_AKUMI_ESP], rect.left, rect.top, rect.right, rect.bottom, vec2_t( 0, 0 ), vec2_t( 1, 1 ), color );
			break;
		case 4:
			render::draw_image( g_NL->m_pIcons[IC_KVA_ESP], rect.left, rect.top, rect.right, rect.bottom, vec2_t( 0, 0 ), vec2_t( 1, 1 ), color );
			break;
		}
	}
	...
	void c_player_esp::oof_indicate ( valve::c_player* player ) {
		...
		auto clr = col_t (
			hacks::g_aim_bot->cfg ( ).m_flOOFArrowsColor[ 0 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flOOFArrowsColor[ 1 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flOOFArrowsColor[ 2 ] * 255.f,
			m_dormant_data.at ( player->index ( ) ).m_alpha * hacks::g_aim_bot->cfg ( ).m_flOOFArrowsColor[ 3 ] );
	...
	void c_player_esp::draw_skeletons ( valve::c_player* player, RECT& rect ) {
		...
		for ( std::size_t i {}; i < mdl_data->m_studio_hdr->m_bones_count; ++i ) {
			...
			render::draw_line ( screen_pos.x, screen_pos.y, screen_parent_pos.x, screen_parent_pos.y, 
				col_t (
					hacks::g_aim_bot->cfg ( ).m_flSkeletonColor[ 0 ] * 255.f,
					hacks::g_aim_bot->cfg ( ).m_flSkeletonColor[ 1 ] * 255.f,
					hacks::g_aim_bot->cfg ( ).m_flSkeletonColor[ 2 ] * 255.f, 
					m_dormant_data.at ( player->index ( ) ).m_alpha ) );
		}
	}
	...
	void c_player_esp::draw_ammo ( valve::c_player* player, RECT& rect ) {
		...
		if ( hacks::g_aim_bot->cfg ( ).m_bAmmo
			...
			auto clr = col_t ( 
				hacks::g_aim_bot->cfg ( ).m_flAmmoColor[ 0 ] * 255.f,
				hacks::g_aim_bot->cfg ( ).m_flAmmoColor[ 1 ] * 255.f,
				hacks::g_aim_bot->cfg ( ).m_flAmmoColor[ 2 ] * 255.f,
				m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flAmmoColor[ 3 ] );
	...
	void c_player_esp::draw_name ( valve::c_player* player, RECT& rect ) {
		...
		render::text ( name, vec2_t ( rect.left + width * 0.5f, rect.top - size.y - 2 ),
			col_t ( 
				hacks::g_aim_bot->cfg ( ).m_flNameColor[ 0 ] * 255.f,
				hacks::g_aim_bot->cfg ( ).m_flNameColor[ 1 ] * 255.f,
				hacks::g_aim_bot->cfg ( ).m_flNameColor[ 2 ] * 255.f,
				m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flNameColor[ 3 ] ),
			hacks::g_visuals->m_fonts.m_04b, FONT_OUTLINE | FONT_CENTERED_X );
	}
	...
	void c_player_esp::draw_weapon ( valve::c_player* player, RECT& rect ) {
		...
		auto clr = col_t (
			hacks::g_aim_bot->cfg ( ).m_flWeaponIconColor[ 0 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flWeaponIconColor[ 1 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flWeaponIconColor[ 2 ] * 255.f,
			m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flWeaponIconColor[ 3 ] );

		auto clr_2 = col_t (
			hacks::g_aim_bot->cfg ( ).m_flWeaponTextColor[ 0 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flWeaponTextColor[ 1 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flWeaponTextColor[ 2 ] * 255.f,
			m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flWeaponTextColor[ 3 ] );

		if ( hacks::g_aim_bot->cfg ( ).m_bWeaponText ) {
			render::text ( player->weapon ( )->get_weapon_name ( ), vec2_t ( rect.left + ( abs ( rect.right - rect.left ) * 0.5f ), rect.bottom + offset + 3 ), clr_2, hacks::g_visuals->m_fonts.m_skeet_font_esp, FONT_OUTLINE | FONT_CENTERED_X );
		...
		if ( hacks::g_aim_bot->cfg ( ).m_bWeaponIcon )
			render::text ( player->weapon ( )->get_weapon_icon ( ), vec2_t ( rect.left + ( abs ( rect.right - rect.left ) * 0.5f ), rect.bottom + offset + 1 ), clr, hacks::g_visuals->m_fonts.m_icon_font, FONT_OUTLINE | FONT_CENTERED_X );
	}
	...
	void c_player_esp::draw_health ( valve::c_player* player, RECT& rect ) {
		...
		col_t color = col_t ( 
			hacks::g_aim_bot->cfg ( ).m_flHealthColor[ 0 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flHealthColor[ 1 ] * 255.f,
			hacks::g_aim_bot->cfg ( ).m_flHealthColor[ 2 ] * 255.f,
			( int ) m_dormant_data [ player->index ( ) ].m_alpha * hacks::g_aim_bot->cfg ( ).m_flHealthColor[ 3 ] );
...
namespace render {
	...
	void draw_image ( ImTextureID texture, float x1, float y1, float x2, float y2, const vec2_t& uv_min, const vec2_t& uv_max, col_t color )
	{
		m_draw_list->AddImage ( texture, ImVec2 ( x1, y1 ), ImVec2 ( x2, y2 ), uv_min, uv_max, color.hex( ) );
	}
	...
	void handle_drawings ( )
	{
		...
		static const auto& callbacks = nem::g_lua->events ( )[  ( "on_paint" ) ];
