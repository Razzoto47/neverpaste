...
namespace nem::hacks {
	void c_skins::init( ) {
		...
		if ( std::ifstream file{ ( "csgo/scripts/items/items_game_cdn.txt" ) } )
		...
		for ( int i{}; i <= item_schema->m_paint_kits.m_last_element; ++i ) {
			...
			if ( pos != std::string::npos
				...
				const auto wpn_name = items.rfind( ( "weapon_" ), pos );
			...
			if ( m_update_hud
				...
				if ( const auto hud_weapons =
					reinterpret_cast< find_t >( g_context->addresses( ).m_find_hud_element.get( ) )(
							g_context->addresses( ).m_hud.get( ), ( "CCSGO_HudWeaponSelection" )
						) - 0x28u )
