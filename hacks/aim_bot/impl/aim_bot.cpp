...
namespace nem::hacks {
	...
	void c_aim_bot::scan_point ( const aim_target_t& target, aim_point_t& point, const bool ignore_dmg, vec3_t shoot_pos ) const {
		...
		const auto intersect = [ ] (
			...
			mat3x4_t temp_matrix = matrix;

			vec3_t temp_min = hitbox->m_min;
			vec3_t temp_max = hitbox->m_max;

			math::vector_transform(temp_min, temp_matrix, min);
			math::vector_transform(temp_max, temp_matrix, max);
	...
	void c_aim_bot::scan_center_points( aim_target_t& target, const int hitgroups, vec3_t shoot_pos ) const {
		...
		for ( std::size_t i{}; i < m_hitgroups.size( ); ++i ) {
			...
			for ( const auto& index : m_hitgroups.at( i ) ) {
				...
				point = math::vector_transform(
					(hitbox->m_min + hitbox->m_max) * 0.5f,
					anim_side.m_bones[hitbox->m_bone]
				);
	...
	void c_aim_bot::calc_capsule_points(
		aim_target_t& target, valve::studio_hitbox_t* hitbox,
		int index, mat3x4_t& matrix, float scale ) const
	{
		...
		static mat3x4_t matrix0;
		vec3_t z_axis(0.f, 0.f, 1.f);
		math::vector_matrix(z_axis, matrix0);

		mat3x4_t matrix1;
		vec3_t dir = (max - min).normalized();
		math::vector_matrix(dir, matrix1);
		...
		for (auto& vertex : points) 
		{
			...
			math::vector_rotate(vertex, matrix0, point);
	...
	void c_aim_bot::calc_multi_points(
		aim_target_t& target, valve::studio_hitbox_t* hitbox,
	    int index, mat3x4_t& matrix, float scale
	) const {
		vec3_t point{};

		auto center = (hitbox->m_min + hitbox->m_max) * 0.5f;
		...
		if ( !should_static_point_scale ( ) ) {
			...
			scale = calc_point_scale( g_eng_pred->spread( ), max, dist.length(), dir, right, up);
			if ( scale <= 0.f
				&& g_eng_pred->spread( ) > g_eng_pred->min_inaccuracy( ) )
				scale = calc_point_scale( g_eng_pred->min_inaccuracy( ), max, dist.length(), dir, right, up );
		}
	...
	void c_aim_bot::scan_points(
		...
		auto& anim_side = target.m_lag_record->m_anim_sides.at( target.m_lag_record->m_anim_side );

		const auto hitbox_set = target.m_entry->m_player->mdl_data( )->m_studio_hdr->hitbox_set( target.m_entry->m_player->hitbox_set_index( ) );

		const auto points_count = calc_points_count( hitgroups, multi_points );
		static auto cl_lagcompensation = valve___g_cvar->find_var (  ( "cl_lagcompensation" ) );
		target.m_points.reserve( points_count );
		...
		for ( std::size_t i{}; i < m_hitgroups.size( ); ++i ) {
			...
			if ( ( ( 1 << i ) & multi_points ) ) {
				...
				for (const auto& index : m_hitgroups.at(i)) {
					...
					vec3_t point{};
					vec3_t center = (hitbox->m_min + hitbox->m_max) * 0.5f;

					mat3x4_t& bone_matrix = anim_side.m_bones[hitbox->m_bone];

					point = math::vector_transform(center, bone_matrix);
	...
	vec3_t get_hitbox ( valve::c_player* ent, std::ptrdiff_t hitbox, valve::bones_t& bones )
	{
	...
	void c_aim_bot::on_create_move( valve::user_cmd_t& user_cmd, bool& send_packet ) {
		...
		auto point = select_point( *target, user_cmd.m_number );
		if ( !point || !point->m_valid )
			return m_targets.clear( );
