...
namespace nem::hacks {
	...
	void c_movement::walk_move(
		...
		if (wishspeed > 0.f && wishspeed > m_max_player_speed)
		{
			wishvel = wishdir.normalized() * m_max_player_speed;
	...
	void c_movement::full_walk_move(
		...
		if ( valve___g_local_player->ground_entity( ) != valve::e_ent_handle::invalid ) {
			...
			if ( speed >= 0.1f ) {
				static auto sv_stopspeed = valve::g_cvar->find_var (  ( "sv_stopspeed" ) )->get_float ( );
	...
	void c_movement::fast_stop ( valve::user_cmd_t& user_cmd ) {
		...
		qangle_t resistance_ang = math::vector_angles(valve::g_local_player->velocity() * -1.f);
