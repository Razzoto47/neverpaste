...
struct ImVec3 {
    ...
    ImVec3(const float* f) : x(f[0]), y(f[1]), z(f[2]) {}
    ...
    ImVec3 operator-(const ImVec3& v) const { return ImVec3(x - v.x, y - v.y, z - v.z); }
    ImVec3 operator*(float s) const { return ImVec3(x * s, y * s, z * s); }
    ImVec3 operator/(float s) const { return ImVec3(x / s, y / s, z / s); }
    ImVec3 operator-() const { return ImVec3(-x, -y, -z); }

    ImVec3& operator+=(const ImVec3& v) { x += v.x; y += v.y; z += v.z; return *this; }
    ImVec3& operator-=(const ImVec3& v) { x -= v.x; y -= v.y; z -= v.z; return *this; }
    ImVec3& operator*=(float s) { x *= s; y *= s; z *= s; return *this; }
    ImVec3& operator/=(float s) { x /= s; y /= s; z /= s; return *this; }

    bool operator>(float s) const { return length_sqr() > s; }
    ImVec3& operator=(float s) { x = s; y = s; z = s; return *this; }

    bool operator&&(const ImVec3& other) const { return !is_zero() && !other.is_zero(); }

    float dot(const ImVec3& v) const { return x * v.x + y * v.y + z * v.z; }
    float dot(const float* v) const { return x * v[0] + y * v[1] + z * v[2]; }

    ImVec3 cross(const ImVec3& v) const {
        return ImVec3(
            y * v.z - z * v.y,
            z * v.x - x * v.z,
            x * v.y - y * v.x
    ...
    float length_sqr_2d() const { return x * x + y * y; }
    ...
    ImVec3 normalized() const {
        float len = length();
        if (len > 0.0f) return *this / len;
        return *this;
    }

    bool operator<(const ImVec3& other) const { return length_sqr() < other.length_sqr(); }
    bool operator<=(const ImVec3& other) const { return length_sqr() <= other.length_sqr(); }
    bool operator>(const ImVec3& other) const { return length_sqr() > other.length_sqr(); }
    bool operator>=(const ImVec3& other) const { return length_sqr() >= other.length_sqr(); }
...
namespace ImGui
{
    ...
    IMGUI_API bool          BeginChildEx(const char* name, int omg, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
