...
#undef ADD_FLAG
				...
				static const auto& callbacks = g_lua->events ( )[  ( "on_create_move" ) ];
...
#undef PROCESS_FLAG
		...
		if ( !send_packet ) {
			...
			g_context->broke_lc( ) = ( valve___g_local_player->origin( ) - g_context->last_sent_origin( ) ).length_sqr_2d( ) > crypt_float ( 4096.f );
	...
	void __fastcall paint_traverse ( void* ecx, void* edx, uint32_t id, bool force_repaint, bool allow_force )
	{
		...
		if ( !panel_name.empty ( ) && !panel_name.compare (  ( "MatSystemTopPanel" ) ) ) {
