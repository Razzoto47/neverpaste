...
{
    int crypted = _byteswap_ulong ( _rotl ( i, rand_val % 31 ) - rand_val );

    volatile int decrypted = crypted;
    decrypted = _byteswap_ulong ( decrypted );
    decrypted += rand_val;
...
public:
    ...
    __forceinline void set ( T val )
    {
        ...
        m_crypted = _byteswap_ulong ( _rotl ( ( unsigned long ) val, ( unsigned long ) ( this ) % 31 ) - ( unsigned long ) ( this ) );
    }

    __forceinline T get ( )
    {
        return ( T ) ( _rotr ( _byteswap_ulong ( m_crypted ) + m_rand_val, m_rol_val ) );
    }
};
