...
namespace nem::valve {
	...
	__forceinline float& c_entity::sim_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_flSimulationTime" ) );
	...
	__forceinline vec3_t& c_entity::origin( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_vecOrigin" ) );
	...
	__forceinline vec3_t& c_entity::abs_origin( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_vecAbsOrigin" ) );
	...
	__forceinline int& c_entity::precip_type ( ) {
		static const auto& net_var = g_net_vars->entry (  ( "CPrecipitation->m_nPrecipType" ) );
	...
	__forceinline vec3_t& c_entity::abs_velocity( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_vecAbsVelocity" ) );
	...
	__forceinline e_ent_flags& c_entity::flags( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_fFlags" ) );
	...
	__forceinline std::uint32_t& c_entity::eflags( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_iEFlags" ) );
	...
	__forceinline std::uint32_t& c_entity::effects( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_fEffects" ) );
	...
	__forceinline int& c_entity::team( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_iTeamNum" ) );
	...
	__forceinline vec3_t& c_entity::obb_min( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_vecMins" ) );
	...
	__forceinline vec3_t& c_entity::obb_max( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_vecMaxs" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CBaseEntity->m_CollisionGroup" ) );
	...
	__forceinline float& c_entity::cycle( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAnimating->m_flCycle" ) );
	...
	__forceinline float& c_entity::anim_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_flAnimTime" ) );
	...
	__forceinline int& c_entity::anim_parity ( ) {
		static const auto& net_var = g_net_vars->entry (  ( "CBaseViewModel->m_nAnimationParity" ) );
	...
	__forceinline e_ent_handle& c_entity::move_parent( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->moveparent" ) );
	...
	__forceinline e_ent_handle& c_entity::rag_doll ( ) {
		static const auto& net_var = g_net_vars->entry (  ( "CCSPlayer->m_hRagdoll" ) );
	...
	__forceinline bool& c_entity::spotted( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_bSpotted" ) );
	...
	__forceinline e_ent_handle& c_entity::thrower( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseGrenade->m_hThrower" ) );
	...
	__forceinline int& c_entity::explode_effect_tick_begin( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCSGrenadeProjectile->m_nExplodeEffectTickBegin" ) );
	...
	__forceinline float& c_entity::grenade_spawn_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCSGrenadeProjectile->m_vecExplodeEffectOrigin" ) );
	...
	__forceinline bool& c_entity::use_fast_path( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAnimating->m_flFrozen" ) );
	...
	__forceinline int& c_entity::bomb_site( ) {
		static const auto& net_var = g_net_vars->entry( ( "CPlantedC4->m_nBombSite" ) );
	...
	__forceinline bool& c_entity::is_bomb_ticking( ) {
		static const auto& net_var = g_net_vars->entry( ( "CPlantedC4->m_bBombTicking" ) );
	...
	__forceinline float& c_entity::c4_blow( ) {
		static const auto& net_var = g_net_vars->entry( ( "CPlantedC4->m_flC4Blow" ) );
	...
	__forceinline vec3_t& c_entity::base_velocity ( ) {
		static const auto& net_var = g_net_vars->entry (  ( "CBasePlayer->m_vecBaseVelocity" ) );
	...
	__forceinline int& c_entity::model_index( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseEntity->m_nModelIndex" ) );
	...
	__forceinline e_ent_handle& c_entity::world_model_handle( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatWeapon->m_hWeaponWorldModel" ) );
	...
	__forceinline int& c_entity::sequence( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseViewModel->m_nSequence" ) );
	...
	__forceinline e_ent_handle& c_entity::view_model_weapon_handle( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseViewModel->m_hWeapon" ) );
	...
	__forceinline e_ent_handle& c_entity::view_model_owner( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseViewModel->m_hOwner" ) );
	...
	__forceinline e_item_index& c_weapon::item_index( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_iItemDefinitionIndex" ) );
	...
	__forceinline float& c_weapon::postpone_fire_ready_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CWeaponCSBase->m_flPostponeFireReadyTime" ) );
	...
	__forceinline float& c_weapon::accuracy_penalty( ) {
		static const auto& net_var = g_net_vars->entry( ( "CWeaponCSBase->m_fAccuracyPenalty" ) );
	...
	__forceinline float& c_weapon::last_shot_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CWeaponCSBase->m_fLastShotTime" ) );
	...
	__forceinline float& c_weapon::recoil_index( ) {
		static const auto& net_var = g_net_vars->entry( ( "CWeaponCSBase->m_flRecoilIndex" ) );
	...
	__forceinline int& c_weapon::ammo( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatWeapon->m_iClip1" ) );
	...
	__forceinline float& c_weapon::next_primary_attack( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatWeapon->m_flNextPrimaryAttack" ) );
	...
	__forceinline float& c_weapon::next_secondary_attack( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatWeapon->m_flNextSecondaryAttack" ) );
	...
	__forceinline float& c_weapon::throw_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCSGrenade->m_fThrowTime" ) );
	...
	__forceinline float& c_weapon::throw_strength( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCSGrenade->m_flThrowStrength" ) );
	...
	__forceinline bool& c_weapon::pin_pulled( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCSGrenade->m_bPinPulled" ) );
	...
	__forceinline bool& c_weapon::in_reload( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_bInReload" ) );
	...
	__forceinline int& c_weapon::zoom_lvl( ) {
		static const auto& net_var = g_net_vars->entry( ( "CWeaponCSBaseGun->m_zoomLevel" ) );
	...
	__forceinline int& c_weapon::item_id_high( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_iItemIDHigh" ) );
	...
	__forceinline int& c_weapon::account_id( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_iAccountID" ) );
	...
	__forceinline int& c_weapon::fallback_paint_kit( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_nFallbackPaintKit" ) );
	...
	__forceinline int& c_weapon::item_quality( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_iEntityQuality" ) );
	...
	__forceinline int& c_weapon::fallback_stat_trak( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_nFallbackStatTrak" ) );
	...
	__forceinline float& c_weapon::fallback_wear( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_flFallbackWear" ) );
	...
	__forceinline int& c_weapon::owner_xuid_low( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_OriginalOwnerXuidLow" ) );
	...
	__forceinline int& c_weapon::owner_xuid_high( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAttributableItem->m_OriginalOwnerXuidHigh" ) );
	...
	__forceinline utl_vec_t< ret_counted_t* >& c_weapon::custom_materials( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatWeapon->m_Item" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_bUseCustomBloomScale" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_bUseCustomAutoExposureMin" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_bUseCustomAutoExposureMax" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_flCustomBloomScale" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_flCustomAutoExposureMin" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CEnvTonemapController->m_flCustomAutoExposureMax" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CCascadeLight->m_envLightShadowDirection" ) );
	...
	__forceinline int& c_player::tick_base( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_nTickBase" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CBasePlayer->m_vphysicsCollisionState" ) );
	...
	__forceinline e_life_state& c_player::life_state( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_lifeState" ) );
	...
	__forceinline int& c_player::health( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_iHealth" ) );
	...
	__forceinline pose_params_t& c_player::pose_params( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAnimating->m_flPoseParameter" ) );
	...
	__forceinline float& c_player::spawn_time( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_iAddonBits" ) );
	...
	__forceinline anim_state_t*& c_player::anim_state( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bIsScoped" ) );
	...
	__forceinline bool& c_player::client_side_anim( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAnimating->m_bClientSideAnimation" ) );
	...
	__forceinline qangle_t& c_player::eye_angles( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_angEyeAngles" ) );
	...
	__forceinline vec3_t& c_player::velocity( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_vecVelocity[0]" ) );
	...
	{
		static const auto& net_var = g_net_vars->entry (  ( "CBasePlayer->m_flFallVelocity" ) );
	...
	__forceinline float& c_player::duck_amount( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_flDuckAmount" ) );
	...
	__forceinline e_ent_handle& c_player::wpn_handle( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatCharacter->m_hActiveWeapon" ) );
	...
	__forceinline e_ent_handle& c_player::ground_entity( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_hGroundEntity" ) );
	...
	__forceinline int& c_player::hitbox_set_index( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseAnimating->m_nHitboxSet" ) );
	...
	__forceinline int& c_player::skin( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_nSkin" ) );
	...
	__forceinline int& c_player::body( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_nBody" ) );
	...
	__forceinline float& c_player::lby( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_flLowerBodyYawTarget" ) );
	...
	__forceinline vec3_t& c_player::view_offset( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_vecViewOffset[0]" ) );
	...
	__forceinline qangle_t& c_player::aim_punch( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_aimPunchAngle" ) );
	...
	__forceinline vec3_t& c_player::aim_punch_vel( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_aimPunchAngleVel" ) );
	...
	__forceinline float& c_player::next_attack( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_flNextAttack" ) );
	...
	__forceinline bool& c_player::ducking( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_bDucking" ) );
	...
	__forceinline bool& c_player::ducked( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_bDucked" ) );
	...
	__forceinline bool& c_player::walking( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bIsWalking" ) );
	...
	__forceinline bool& c_player::strafing( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bStrafing" ) );
	...
	__forceinline e_move_type& c_player::move_type( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_MoveType" ) );
	...
	__forceinline float& c_player::velocity_modifier( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_flVelocityModifier" ) ); // physics_coll_state in data map is velocity_modifier
	...
	__forceinline qangle_t& c_player::view_punch( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_viewPunchAngle" ) );
	...
	__forceinline float& c_player::duck_speed( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_flDuckSpeed" ) );
	...
	__forceinline int& c_player::move_state( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_iMoveState" ) );
	...
	__forceinline float& c_player::third_person_recoil( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_flThirdpersonRecoil" ) );
	...
	__forceinline int& c_player::armor_value( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_ArmorValue" ) );
	...
	__forceinline bool& c_player::has_helmet( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bHasHelmet" ) );
	...
	__forceinline bool& c_player::has_heavy_armor( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bHasHeavyArmor" ) );
	...
	__forceinline float& c_player::max_speed( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_flMaxspeed" ) );
	...
	__forceinline float& c_player::surface_friction( ) {
		static const auto& net_var = g_net_vars->entry( ( "m_surfaceFriction" ) );
	...
	__forceinline bool& c_player::immune( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_bGunGameImmunity" ) );
	...
	__forceinline e_ent_handle c_player::view_model_handle( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_hViewModel[0]" ) );
	...
	__forceinline int& c_player::money( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_iAccount" ) );
	...
	__forceinline int& c_player::observer_mode( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_iObserverMode" ) );
	...
	__forceinline e_ent_handle& c_player::observer_target( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBasePlayer->m_hObserverTarget" ) );
	...
	__forceinline e_ent_handle* c_player::weapons( ) {
		static const auto& net_var = g_net_vars->entry( ( "CBaseCombatCharacter->m_hMyWeapons" ) );
	...
	__forceinline float& c_player::flash_duration( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_flFlashDuration" ) );
	...
	__forceinline int& c_player::shots_fired( ) {
		static const auto& net_var = g_net_vars->entry( ( "CCSPlayer->m_iShotsFired" ) );
